...
Bart = {
    initWatcher : () => {
        ...
        watcherSubscription = Observable.create(observer => {
            ...
            const fileExtentions = `${path.sep}**${path.sep}*.{png,gif,jpg,js,ds,isml,xml,json,properties,svg,ttf,eot,woff,css}`;
            ...
            atom.project.getDirectories().forEach(dir => {
                if (options.cartridges && options.cartridges.length) {
                    options.cartridges
                        .map(str => str.trim())
                        .filter(Boolean)
                        .forEach((cartridge, index) => {
                            if (!found.includes(index)) {
                                fs.stat(path.join(dir.getPath(), cartridge), (err, stats) => {
    ...
    getOptions : () => {
        ...
        if (typeof configObj.cartridges === 'string') {
          configObj.cartridges = configObj.cartridges.indexOf(',') > -1 ?
            configObj.cartridges.split(',') : configObj.cartridges.trim().split('\n');
      }
    ...
    saveFile : (fileName /*: string*/) => {
        ...
        if (!currentRoot) {
            atom.notifications.addWarning('File is not in cartridge list to be uploaded');
            return;
        }
        ...
        uploadCartridgesSubscription = Observable.concat(...atom.project.getDirectories().map(dir => {
            ...
            if (options.cartridges && options.cartridges.length) {
                const toUpload = options.cartridges
                    ...
                    .filter((cartridge) => {
                        try {
                            fs.statSync(path.join(dir.getPath(), cartridge));
                            return true;
                        } catch (err) {
                            return false;
                        }
            ...
            () => {
                ...
                uploadCartridgesSubscription = null;
                Bart.initWatcher();
                atom.notifications.addInfo('Cartridges have been uploaded.');
            }
        );
    },
