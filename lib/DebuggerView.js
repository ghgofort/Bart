...
class BreakpointView extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    removeBreakpoint(key) {
        this.setState((prevState, props) => {
            breakpoints: prevState.breakpoints.splice(prevState.breakpoints.indexOf(key), 1);
        });
    }
    toggleBreakpoint(key) {
        let breakPts = this.state.breakpoints;
        breakPts[key].checked = !this.state.breakpoints[key].checked;
    ...
    render() {
        const breakpoints = this.state.breakpoints.map((bp) => {
            <label>{bp.lineNumber + ' ' + bp.codeSummary}<input type='checkbox' checked={bp.selected}/>
        });
        return(
            <div id="bart-breakpoints">
                <ul>
                    {breakpoints}
                </ul>
            </div>
